...
class MolecularEncoder:
    ...
    def collate(
        ...
        encodings["input_ids"] = [
            torch.from_numpy(np.array([x[0] for x in encodings["input_ids"]])).to(device),
            torch.from_numpy(np.array([x[1] for x in encodings["input_ids"]])).to(device),
        ]
        encodings["position_ids"] = torch.from_numpy(np.array(encodings["position_ids"])).to(device)
        encodings["attention_mask"] = torch.from_numpy(np.array(encodings["attention_mask"])).to(device)
        encodings["attention_type_ids"] = torch.from_numpy(np.array(encodings["attention_type_ids"])).to(device)

        if "labels" in encodings:
            encodings["labels"] = torch.from_numpy(encodings["labels"]).to(device)
